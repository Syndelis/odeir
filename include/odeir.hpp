...
template<>
ModelBuilder<PopulationState> ModelBuilder<NodeState>::withRelatedConstant(std::string relatedConstantName) {
...
template<>
ModelBuilder<LinkState> ModelBuilder<PopulationState>::addLink(NodeId id, char sign) {
...
template<>
void *ModelBuilder<LinkState>::end() {
    internal_api::odeir_insert_population_link(state.population, state.sign, state.id);
    return nullptr;
}

template<>
ModelBuilder<LinkState> ModelBuilder<LinkState>::addLink(NodeId id, char sign) {
    this->end();
    return ModelBuilder<LinkState>(
...
template<>
ModelBuilder<CombinatorState> ModelBuilder<NodeState>::withOperation(char op) {
...
template<>
void *ModelBuilder<InputState>::end() {
...
template<>
ModelBuilder<InputState> ModelBuilder<CombinatorState>::addInput(NodeId id) {
    auto node = static_cast<internal_api::Node *>(this->end());
...
template<>
ModelBuilder<InputState> ModelBuilder<InputState>::addInput(NodeId id) {
    this->end();
    return ModelBuilder<InputState>(
